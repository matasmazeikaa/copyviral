...
// Types matching your app's types
interface MediaFile {
    id: string;
    fileName: string;
    fileId: string;
    ...
    startTime: number;
    src?: string;
    supabaseFileId?: string; // For files stored in Supabase Storage
...
export const handler = async (event: SQSEvent) => {
    for (const record of event.Records) {
        ...
        try {
            ...
            const input: RenderJobInput = job.input_data;
            ...
            try {
                ...
                execSync(`${FFMPEG_PATH} ${ffmpegCmd}`, {
                    cwd: TMP_DIR,
            ...
            try {
                execSync(
                    `${FFMPEG_PATH} -i "${outputPath}" -ss ${thumbnailTime.toFixed(2)} -vframes 1 -q:v 2 -y "${thumbnailPath}"`,
                    {
                        cwd: TMP_DIR,
...
async function downloadMediaFiles(mediaFiles: MediaFile[], userId: string): Promise<Map<string, string>> {
...
function hasAudioStream(filePath: string): boolean {
    try {
        ...
        const result = execSync(
            `/opt/bin/ffprobe -v error -select_streams a -show_entries stream=codec_type -of csv=p=0 "${filePath}"`,
...
function buildFfmpegCommand(input: RenderJobInput, localFiles: Map<string, string>): string {
    ...
    const overlays: { label: string; x: number; y: number; start: number; end: number }[] = [];
    ...
    for (const file of sortedFiles) {
        ...
        if (file.type === 'video') {
            filters.push(
                `[${i}:v]trim=start=${file.startTime.toFixed(3)}:duration=${duration.toFixed(3)},${scaleFilter},setpts=PTS-STARTPTS+${file.positionStart.toFixed(3)}/TB${opacityFilter}[v${i}]`
            );
        } else if (file.type === 'image') {
            filters.push(
                `[${i}:v]${scaleFilter},setpts=PTS+${file.positionStart.toFixed(3)}/TB${opacityFilter}[v${i}]`
            );
        }
        ...
        if (file.type === 'video' || file.type === 'audio') {
            ...
            if (fileHasAudio) {
                ...
                filters.push(
                    `[${i}:a]atrim=start=${file.startTime.toFixed(3)}:duration=${duration.toFixed(3)},asetpts=PTS-STARTPTS,adelay=${delayMs}|${delayMs},volume=${volumeLinear.toFixed(4)}[a${i}]`
                );
    ...
    if (overlays.length > 0) {
        for (let i = 0; i < overlays.length; i++) {
            ...
            filters.push(
                `[${lastLabel}][${label}]overlay=${x}:${y}:enable='between(t\\,${start.toFixed(3)}\\,${end.toFixed(3)})'[${nextLabel}]`
            );
            lastLabel = nextLabel;
        }
    } else {
    ...
    if (textElements.length > 0) {
        ...
        const textLines: { text: TextElement; line: string; lineIndex: number; totalLines: number }[] = [];
    ...
    if (needsWatermark && watermarkInputIndex !== null) {
        ...
        filters.push(
            `[${watermarkInputIndex}:v]scale=${watermarkScale}:-1,format=rgba,colorchannelmixer=aa=0.8[wm]`
        );
        
        // Overlay watermark at right side, positioned within typical content area
        filters.push(
            `[preWm][wm]overlay=W-w-${paddingX}:${watermarkY}[outv]`
        );
    ...
    switch (exportSettings.quality) {
        ...
        case 'ultra': crf = 14; break;
        default: crf = 23;
    }
    
    switch (exportSettings.speed) {
        case 'fastest': preset = 'ultrafast'; break;
        case 'fast': preset = 'veryfast'; break;
        case 'balanced': preset = 'medium'; break;
        case 'slow': preset = 'slow'; break;
        case 'slowest': preset = 'veryslow'; break;
