...
function validateCreateCommunityTemplate(body: any): ValidationError[] {
    const errors: ValidationError[] = [];
    ...
    if (body.description !== undefined && body.description !== null) {
        const descError = validateString(body.description, 'description', 0, 500);
    ...
    if (body.sourceUrl !== undefined && body.sourceUrl !== null) {
        const urlError = validateUrl(body.sourceUrl, 'sourceUrl');
        if (urlError) errors.push(urlError);
    }
    
    // Optional: viewCount
    if (body.viewCount !== undefined) {
        const viewError = validateNumber(body.viewCount, 'viewCount', 0);
        if (viewError) errors.push(viewError);
    }
    
    // Optional: category
    if (body.category !== undefined && body.category !== null) {
        const catError = validateString(body.category, 'category', 1, 50);
...
// Use Supabase service role key for admin authentication
const SERVICE_ROLE_KEY = process.env.SUPABASE_SERVICE_ROLE_KEY;

/**
 * Verify service role key from request headers
 * Admin routes require the x-service-role-key header
 */
function verifyAdminKey(request: NextRequest): boolean {
    const apiKey = request.headers.get('x-service-role-key');
    if (!SERVICE_ROLE_KEY) {
 ...
 * List all active community templates (public access)
 */
 ...
 * Create a new community template (admin only)
 */
export async function POST(request: NextRequest) {
    try {
        ...
        const errors = validateCreateCommunityTemplate(body);
        ...
        console.error('POST community template error:', error);
        return NextResponse.json(
            { error: "Internal server error" },
            { status: 500 }
        );
...
/**
 * PUT /api/templates/community
 * Update a community template (admin only)
 */
export async function PUT(request: NextRequest) {
    try {
        ...
        if (error) {
            console.error('Error updating community template:', error);
            return NextResponse.json(
        ...
        console.error('PUT community template error:', error);
        return NextResponse.json(
            { error: "Internal server error" },
            { status: 500 }
        );
...
/**
 * DELETE /api/templates/community
 * Delete a community template (admin only)
 */
export async function DELETE(request: NextRequest) {
    try {
        ...
        if (error) {
            console.error('Error deleting community template:', error);
            return NextResponse.json(
